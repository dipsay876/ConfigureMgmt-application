...
public class ComponentMasterResource {
    ...
    public ResponseEntity<List<ComponentMaster>> getAllComponentMasters(Pageable pageable, @RequestParam(required = false) String filter) {
        if ("componentname-is-null".equals(filter)) {
            log.debug("REST request to get all ComponentMasters where componentName is null");
            return new ResponseEntity<>(componentMasterService.findAllWhereComponentNameIsNull(),
                    HttpStatus.OK);
        }
